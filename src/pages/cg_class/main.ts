...
import {srcs as SCIMG} from "../../ts/ImportSCImage.ts"
import CLASS_IMAGE from "../../img/other/class.jpg"
import { DrawImage, PutImgToAry, SetLoadEvent, getUrlParamSet } from "../../ts/CanvasUtil.ts";

var images = [] as Array<CanvasImage>;

/** バスの座席表 */
const classImg = new CanvasImage();
classImg.img = new Image();
classImg.index = 0;
classImg.img.src = CLASS_IMAGE;
classImg.drawOffsetX = 0;
classImg.drawOffsetY = 0;
classImg.drawHeight = 767;
classImg.drawWidth = 800;
...
function initCharImg(context)
{
	...
	for (var j in images) 
    {
        ...
        const lineNum = 14;
        images[j].drawOffsetX = x + ( (images[j].index%lineNum) * 50) + classImg.drawWidth;
...
export function init(canvas) 
{
	...
	//デレマスアイドルの追加
	PutImgToAry(srcs,images);

	//シャニマスアイドルの追加
	PutImgToAry(SCIMG,images);

	//画像を全て読み込んだときにキャンバスを再描画するように修正
	var loadCounter = 0;
	SetLoadEvent(images,upsetImages,canvas,context,loadCounter);
